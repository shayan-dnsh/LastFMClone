...
/// `CacheRequestManager` It can be used for cache netwrok response if netwrok request caching needed
public enum CacheRequestManagerError: AppFailures {
    case invalidFileName(urlString: String)
    case invalidFileData
}

open class CacheRequestManager {
    public static var sharedInstance = CacheRequestManager()
           ...
           let url = self.fileURL(fileName: fileName) {
            if let data = try? Data(contentsOf: url),
               let dictionary = try? NSKeyedUnarchiver.unarchiveTopLevelObjectWithData(data) {
                ...
                throw CacheRequestManagerError.invalidFileData
            }
        } else {
            throw CacheRequestManagerError.invalidFileName(urlString: urlString)
        }
    }
